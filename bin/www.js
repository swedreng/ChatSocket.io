...
io.sockets.on('connection', function(socket){
           ...
           if(ind !== -1){
                ...
                var msg = msg.substring(ind + 1);
                    if(name in users){
                      ...
                      var conn = new sql.ConnectionPool(dbConn);
      ...
      if(ind !== -1){
        ...
        var msg = msg.substring(ind + 1);
           if(true){
            ...
            var con = new sql.Request(conn);
            con.query("INSERT into CevrimdisiMesajlarr (cevrimdisiMesajlar_kim,cevrimdisiMesajlar_kime,cevrimdisiMesajlar_mesaj) values ('"+socket.username+"',\'"+name+"',\'"+msg+"')").then(function (recordset) {
              conn.close();
  ...
  socket.on('new user',function(data,callback){
    ...
    conn.connect().then(function () {
      ...
      var con = new sql.Request(conn);
      console.log(data);
      con.query("SELECT * FROM uye WHERE uye_username='"+data+"'").then(function (result){
      ...
      if(data in users){
        ...
        if(a){
          ...
          var con = new sql.Request(conn);
            ...
            con.query("SELECT * FROM CevrimdisiMesajlarr WHERE cevrimdisiMesajlar_kime='"+socket.username+"'").then(function (result){

              conn.close();
              if(result.recordset.length > 0){
                socket.emit('cevrimdisiMesaj',result.recordset,function(){
  ...
  socket.on('user kay',function(data,callback){
    ...
    conn.connect().then(function () {
      ...
      con.query("INSERT into uye (uye_username,uye_password) values ('"+data.username+"',\'"+data.password+"')").then(function (recordset) {
        conn.close();
  ...
  socket.on('get not online users',function(data,callback){
    ...
    var con = new sql.Request(conn);
      ...
      con.query("SELECT * FROM uye").then(function (result){

        conn.close();

        if(result.recordset.length > 0){
          socket.emit('get not online users',result.recordset,function(){
